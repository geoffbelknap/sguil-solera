...
proc SoleraEvent { } {
	...
	global SOLERA_HOST SOLERA_USER SOLERA_PASS
    if {!$ACTIVE_EVENT} {return}
    set selectedIndex [$CUR_SEL_PANE(name) curselection]
    set sidcidList [split [$CUR_SEL_PANE(name) getcells $selectedIndex,alertID] .]
    set cnxID [lindex $sidcidList 1]
    ...
    set startTime [clock scan "5 minute" -base [clock scan $timestamp -gmt 1]]
    set endTime   [clock scan "-5 minute" -base [clock scan $timestamp -gmt 1]]
    set startTime [clock format $startTime -format {%m.%d.%Y.%H.%M.%S} -gmt 1]
    set endTime [clock format $endTime -format {%m.%d.%Y.%H.%M.%S} -gmt 1]
	set queryString "https://$SOLERA_HOST/deepsee_reports?user=$SOLERA_USER&password=$SOLERA_PASS#pathString=%2Ftimespan%2F${startTime}.${endTime}%2Fipv4_address%2F${srcIP}_and_${dstIP}"
	switch $proto {
		"6" {
			# TCP
			set queryString2 "%2Ftcp_port%2F${srcPort}_and_${dstPort}%2F;reportIndex=0"
		}
		"17" {
			# UDP
			set queryString2 "%2Fudp_port%2F${srcPort}_and_${dstPort}%2F;reportIndex=0"
		}
		"1" {
			# ICMP
			set queryString2 "%2Fip_protocol%2Ficmp%2F;reportIndex=0"
		}
		default {
			# All Other Protocols
			set queryString2 "%2F;reportIndex=0"
		}
	}
	exec $BROWSER_PATH $queryString$queryString2 &
}

proc SoleraQuery { report_type ips skew } {
    global ACTIVE_EVENT SERVERHOST XSCRIPT_SERVER_PORT DEBUG CUR_SEL_PANE XSCRIPTDATARCVD
    global socketWinName SESSION_STATE WIRESHARK_STORE_DIR WIRESHARK_PATH BROWSER_PATH
	global SOLERA_HOST SOLERA_USER SOLERA_PASS
    if {!$ACTIVE_EVENT} {return}
	...
	# Set Time Skew (in Hours)
     set startTime	[clock scan "$skew hour" -base [clock scan $timestamp -gmt 1]]
     set endTime	[clock scan "-$skew hour" -base [clock scan $timestamp -gmt 1]]
	# Re-Format Time into Solera 'Timespan' Format
    set startTime [clock format $startTime -format {%m.%d.%Y.%H.%M.%S} -gmt 1]
    set endTime [clock format $endTime -format {%m.%d.%Y.%H.%M.%S} -gmt 1]

	set queryStringSetup "https://$SOLERA_HOST/deepsee_reports?user=$SOLERA_USER&password=$SOLERA_PASS#pathString=%2Ftimespan%2F${startTime}.${endTime}"
	...
	exec $BROWSER_PATH $queryStringSetup$queryIPs$queryType &
}

proc SoleraArtifactEvent { } {
    global ACTIVE_EVENT SERVERHOST XSCRIPT_SERVER_PORT DEBUG CUR_SEL_PANE XSCRIPTDATARCVD
    global socketWinName SESSION_STATE WIRESHARK_STORE_DIR WIRESHARK_PATH BROWSER_PATH
	global SOLERA_HOST SOLERA_USER SOLERA_PASS
    if {!$ACTIVE_EVENT} {return}
    ...
    set startTime [clock format $startTime -format {%m.%d.%Y.%H.%M.%S} -gmt 1]
    set endTime [clock format $endTime -format {%m.%d.%Y.%H.%M.%S} -gmt 1]
	set queryString "https://$SOLERA_HOST/ws/artifact-search-flex?user=$SOLERA_USER&password=$SOLERA_PASS#path=%2Ftimespan%2F${startTime}.${endTime}%2Fipv4_address%2F${srcIP}_and_${dstIP}"
	switch $proto {
		"6" {
			# TCP
			set queryString2 "%2Ftcp_port%2F${srcPort}_and_${dstPort}%2F"
		}
		"17" {
			# UDP
			set queryString2 "%2Fudp_port%2F${srcPort}_and_${dstPort}%2F"
		}
		"1" {
			# ICMP
			set queryString2 "%2Fip_protocol%2Ficmp%2F"
		}
		default {
			# All Other Protocols
			set queryString2 "%2F"
